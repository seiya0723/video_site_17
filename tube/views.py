...
from .models import Video,Category,VideoComment,VideoCommentReply,VideoCommentReplyToReply,MyList,History,GoodVideo

from users.models import CustomUser,FollowUser,BlockUser

from .serializer import VideoSerializer,VideoEditSerializer,VideoCommentSerializer,VideoCommentEditSerializer,VideoCommentReplyEditSerializer,VideoCommentReplySerializer,VideoCommentReplyToReplySerializer,MyListSerializer,HistorySerializer,RateSerializer,GoodSerializer,IconSerializer,FollowUserSerializer,BlockUserSerializer,UserInformationSerializer
...
# コメントの削除、編集
class VideoCommentEditView(views.APIView):
...
# 動画コメントのリプライの削除、編集
class VideoCommentReplyEditView(views.APIView):
...
class VideoCommentReplyView(views.APIView):
...
class VideoCommentReplyToReplyView(views.APIView):

    def get(self, request, videocommentreply_pk, *args, **kwargs):

        json = {"error": True, }

        replies = VideoCommentReplyToReply.objects.filter(target=videocommentreply_pk).order_by("-dt")
...
class UserPolicyView(views.APIView):
    ...
    def get(self,request,*args,**kwargs):
        ...
        return render(request,"tube/user_policy.html")
...
class UserSingleView(LoginRequiredMixin,views.APIView):
    ...
    def get(self, request, pk, *args, **kwargs):
        ...
        #TODO:idではなくtargetで絞り込む、annotateも追加する。
        #videos  = Video.objects.filter(id=pk)
        videos  = Video.objects.annotate(num_comments=Count("videocomment", distinct=True),num_mylists=Count("mylist", distinct=True)).filter(user=pk)
